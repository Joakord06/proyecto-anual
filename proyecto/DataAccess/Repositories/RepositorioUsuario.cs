...
namespace DataAccess.Repositories
{
    public class UserRepository
    {
        ...
        public void DeleteSecurityQuestionsForUser(int userId)
        {
            using var conn = new SqlConnection(_connectionString);
            conn.Open();
            using var cmd = new SqlCommand("DELETE FROM SecurityQuestions WHERE UserId=@userId", conn);
            cmd.Parameters.AddWithValue("@userId", userId);
            cmd.ExecuteNonQuery();
        }

        public void AddSecurityQuestion(int userId, string question, string answerHash)
        {
            using var conn = new SqlConnection(_connectionString);
            conn.Open();
            using var cmd = new SqlCommand("INSERT INTO SecurityQuestions (UserId, Question, AnswerHash) VALUES (@userId,@q,@a)", conn);
            cmd.Parameters.AddWithValue("@userId", userId);
        ...
        public IEnumerable<User> GetAllUsers()
        {
            ...
            using var conn = new SqlConnection(_connectionString);
            conn.Open();
            using var cmd = new SqlCommand("SELECT Id, Username, PasswordHash, Email, IsFirstLogin, Role, CreatedAt FROM Users ORDER BY Username", conn);
            using var r = cmd.ExecuteReader();
            while (r.Read())
            {
                list.Add(new User
