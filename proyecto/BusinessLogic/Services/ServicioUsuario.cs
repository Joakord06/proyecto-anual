...
namespace LayeredApp.Business.Services
{
    public class UserService : IUserService
    {
        ...
        public void SaveSecurityQuestions(int userId, IEnumerable<(string Question, string AnswerPlain)> questions)
        {
            // delete existing and insert new hashed answers
            _repo.DeleteSecurityQuestionsForUser(userId);
            foreach (var (Question, AnswerPlain) in questions)
            {
                var aHash = HashAnswer(AnswerPlain);
                _repo.AddSecurityQuestion(userId, Question, aHash);
            }
        }

        public IEnumerable<(int Id, string Question)> GetSecurityQuestionsForUser(int userId)
        {
            var q = _repo.GetQuestionsForUser(userId);
            return q.Select(x => (x.Id, x.Question));
        }
    }
}
